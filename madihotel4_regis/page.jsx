...
import hotelstyle from '@/app/madinastyle/hotel4_regis.module.css';

import Link from 'next/link';

export default function mdhotel4() {
...
const calculation = () => {
  ...
  if (selectedOption === " Price(PKR 59,997) + PKR 12,449 Taxes.") {
    
      const twosingle = jsonData.mdhotel4.two_single_beds + jsonData.mdhotel4.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption ===" Price(PKR 62,997 ) + PKR 13,072 Taxes."){
      const threebed_room = jsonData.mdhotel4.three_single_beds + jsonData.mdhotel4.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Price(PKR 65,997) + PKR 13,072 Taxes."){
      const fourbed_room = jsonData.mdhotel4.four_single_beds + jsonData.mdhotel4.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption ===" Price(PKR 68,997) +PKR 14,317 Taxes."){
      const fivebed_room = jsonData.mdhotel4.five_single_beds + jsonData.mdhotel4.five_single_beds_taxes;
    ...
    <h1> -o( Le Bosphorus Al Madinah )o-</h1>
     ...
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Price(PKR 59,997) + PKR 12,449 Taxes." >2 Single bed's (PKR 59,997) </option>
        <option value=" Price(PKR 62,997 ) + PKR 13,072 Taxes." >Classic Triple Room (PKR 62,997) </option>
        <option value=" Price(PKR 65,997) + PKR 13,072 Taxes." >Quadruple Room (PKR 65,997) </option>
        <option value=" Price(PKR 68,997) +PKR 14,317 Taxes." >5 Single bed's (PKR 68,997)</option>
      </select>
        </div>
        <br></br>
        <div >
