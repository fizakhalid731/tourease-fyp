...
import hotelstyle from '@/app/makkahstyles/regis_hotel7.module.css';

import Link from 'next/link';

export default function Makhotel7_regis() {
...
const calculation = () => {
  ...
  if (selectedOption === " 2 single bed's,  Price(PKR 43,371) + PKR 8,676   Taxes." ) {
    
      const twosingle = jsonData.hotel7.two_single_beds + jsonData.hotel7.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption === " 3 single bed's,  Price(PKR 163,298 ) + PKR 33,884  Taxes." ){
      const threebed_room = jsonData.hotel7.three_single_beds + jsonData.hotel7.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Quadruple Room,  Price(PKR 59,626) + PKR 12,372    Taxes." ){
      const fourbed_room = jsonData.hotel7.four_single_beds + jsonData.hotel7.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption === " Family room,  Price(PKR 489,893) +PKR 101.653   Taxes." ){
      const fivebed_room = jsonData.hotel7.five_single_beds + jsonData.hotel7.five_single_beds_taxes;
    ...
    <h1> -o( Al Safwah Royale Orchid Hotel )o-</h1>
    </header>
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" 2 single bed's,  Price(PKR 43,371) + PKR 8,676   Taxes." >2 single bed's (PKR 43,371) </option>
        <option value=" 3 single bed's,  Price(PKR 163,298 ) + PKR 33,884  Taxes." >3 single bed's, Breakfast included (PKR 163,298) </option>
        <option value=" Quadruple Room,  Price(PKR 59,626) + PKR 12,372    Taxes." >4 single bed's (PKR 59,626) </option>
        <option value=" Family room,  Price(PKR 489,893) +PKR 101.653   Taxes." >Family room, Breakfast included (PKR 489,893)</option>
      </select>
        </div>
        <br></br>
        <div >
