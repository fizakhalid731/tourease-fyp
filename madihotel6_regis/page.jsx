...
import hotelstyle from '@/app/madinastyle/hotel6_regis.module.css';

import Link from 'next/link';

export default function mdhotel6() {
...
const calculation = () => {
  ...
  if (selectedOption === " Price(PKR 63,331) + PKR 13,141 Taxes.") {
    
      const twosingle = jsonData.mdhotel6.two_single_beds + jsonData.mdhotel6.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption ===" Price(PKR 67,997 ) + PKR 14,109 Taxes."){
      const threebed_room = jsonData.mdhotel6.three_single_beds + jsonData.mdhotel6.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption ===" Price(PKR 74,330) + PKR 15,424 Taxes."){
      const fourbed_room = jsonData.mdhotel6.four_single_beds + jsonData.mdhotel6.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption ===" Price(PKR 127,995) +PKR 26,559 Taxes."){
      const fivebed_room = jsonData.mdhotel6.five_single_beds + jsonData.mdhotel6.five_single_beds_taxes;
    ...
    <h1> -o( Pullman Zamzam Madina )o-</h1>
    </header>
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Price(PKR 63,331) + PKR 13,141 Taxes." >1 extra large double bed (PKR 63,331) </option>
        <option value=" Price(PKR 67,997 ) + PKR 14,109 Taxes." >Classic Triple Room (PKR 67,997) </option>
        <option value=" Price(PKR 74,330) + PKR 15,424 Taxes." >Quadruple Room (PKR 74,330) </option>
        <option value=" Price(PKR 127,995) +PKR 26,559 Taxes." >extra-large double & 3 Single bed's (PKR 127,995)</option>
      </select>
        </div>
        <br></br>
        <div >
