...
import hotelstyle from '@/app/makkahstyles/regis_hotel2.module.css';
...
const calculation = () => {
  ...
  if (selectedOption === " Twin Room,  Price(PKR 31,676) + PKR 5709 Taxes." ) {
    
      const twosingle = jsonData.hotel2.two_single_beds + jsonData.hotel2.two_single_beds_taxes; 
  ...
  else if(selectedOption === " 3 single bed's,  Price(PKR 73,484 ) + PKR 5511 Taxes." ){
      const threebed_room = jsonData.hotel2.three_single_beds + jsonData.hotel2.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Quadruple Room,  Price(PKR 39,129) + PKR 7053 Taxes." ){
      const fourbed_room = jsonData.hotel2.four_single_beds + jsonData.hotel2.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption === " Family room,  Price(PKR 92,783) +PKR 6958  Taxes."){
      const fivebed_room = jsonData.hotel2.five_single_beds + jsonData.hotel2.five_single_beds_taxes;
    ...
    <h1> -o( Al Tayseer Towers Tuwa )o-</h1>
    </header>
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Twin Room,  Price(PKR 31,676) + PKR 5709 Taxes." >Twin Room (PKR 31,676) </option>
        <option value=" 3 single bed's,  Price(PKR 73,484 ) + PKR 5511 Taxes." >3 single bed's (PKR 73,484) </option>
        <option value=" Quadruple Room,  Price(PKR 39,129) + PKR 7053 Taxes." >Quadruple Room /4 single bed's (PKR 39,129) </option>
        <option value=" Family room,  Price(PKR 92,783) +PKR 6958  Taxes." >Family room /5 single bed's (PKR 92,783)</option>
      </select>
        </div>
        <br></br>
        <div >
