...
import hotelstyle from '@/app/madinastyle/hotel3_regis.module.css';

import Link from 'next/link';

export default function mdhotel3() {
...
const calculation = () => {
  ...
  if (selectedOption === " Price(PKR 64,812) + PKR 11,585 Taxes.") {
    
      const twosingle = jsonData.mdhotel3.two_single_beds + jsonData.mdhotel3.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption ===" Price(PKR 81,478 ) + PKR 14,564 Taxes."){
      const threebed_room = jsonData.mdhotel3.three_single_beds + jsonData.mdhotel3.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Price(PKR 92,589) + PKR 16,550 Taxes."){
      const fourbed_room = jsonData.mdhotel3.four_single_beds + jsonData.mdhotel3.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption === " Price(PKR 146,290) +PKR 26,290 Taxes."){
      const fivebed_room = jsonData.mdhotel3.five_single_beds + jsonData.mdhotel3.five_single_beds_taxes;
    ...
    <h1> -o( Le Bosphorus Al Madinah )o-</h1>
    </header>
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Price(PKR 64,812) + PKR 11,585 Taxes." >1 extra alrge double bed (PKR 64,812) </option>
        <option value=" Price(PKR 81,478 ) + PKR 14,564 Taxes." >Classic Triple Room (PKR 81,478) </option>
        <option value=" Price(PKR 92,589) + PKR 16,550 Taxes." >Quadruple Room (PKR 92,589) </option>
        <option value=" Price(PKR 146,290) +PKR 26,290 Taxes." >extra-large double & 3 Single bed's (PKR 146,290)</option>
      </select>
        </div>
        <br></br>
        <div >
