...
import hotelstyle from '@/app/makkahstyles/regis_hotel8.module.css';

import Link from 'next/link';

export default function Makhotel8_regis() {
...
const calculation = () => {
  ...
  if (selectedOption === " 2 single bed's,  Price(PKR 44,684) + PKR 2,234 Taxes." ) {
    
      const twosingle = jsonData.hotel8.two_single_beds + jsonData.hotel8.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption === " 3 single bed's,  Price(PKR 52,478 ) + PKR 2,623 Taxes." ){
      const threebed_room = jsonData.hotel8.three_single_beds + jsonData.hotel8.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Quadruple Room,  Price(PKR 86,028) + PKR 4,301   Taxes." ){
      const fourbed_room = jsonData.hotel8.four_single_beds + jsonData.hotel8.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption === " Family room,  Price(PKR 255,115) +PKR 12,755  Taxes." ){
      const fivebed_room = jsonData.hotel8.five_single_beds + jsonData.hotel8.five_single_beds_taxes;
    ...
    <h1> -o( Anjum Makkah Hotel )o-</h1>
     ...
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" 2 single bed's,  Price(PKR 44,684) + PKR 2,234 Taxes." >2 single bed's (PKR 44,684) </option>
        <option value=" 3 single bed's,  Price(PKR 52,478 ) + PKR 2,623 Taxes." >3 single bed's (PKR 52,478) </option>
        <option value=" Quadruple Room,  Price(PKR 86,028) + PKR 4,301   Taxes." >4 single bed's (PKR 86,028) </option>
        <option value=" Family room,  Price(PKR 255,115) +PKR 12,755  Taxes." >Family room /5 single bed's (PKR 255,115)</option>
      </select>
        </div>
        <br></br>
        <div >
