...
import hotelstyle from '@/app/madinastyle/hotel8_regis.module.css';

import Link from 'next/link';

export default function mdhotel8() {
...
const calculation = () => {
  ...
  if (selectedOption === " Price(PKR 39,998) + PKR 5,720  Taxes.") {
    
      const twosingle = jsonData.mdhotel8.two_single_beds + jsonData.mdhotel8.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption ===" Price(PKR 41,998 ) + PKR 6,006  Taxes."){
      const threebed_room = jsonData.mdhotel8.three_single_beds + jsonData.mdhotel8.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption ===" Price(PKR 43,998) + PKR 6,292  Taxes."){
      const fourbed_room = jsonData.mdhotel8.four_single_beds + jsonData.mdhotel8.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption ===" Price(PKR 81,996 ) +PKR 11,725  Taxes."){
      const fivebed_room = jsonData.mdhotel8.five_single_beds + jsonData.mdhotel8.five_single_beds_taxes;
    ...
    <h1> -o( Al Aqeeq Madinah Hotel )o-</h1>
     ...
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Price(PKR 39,998) + PKR 5,720  Taxes." >1 large double bed (PKR 39,998) </option>
        <option value=" Price(PKR 41,998 ) + PKR 6,006  Taxes." >Classic Triple Room (PKR 41,998) </option>
        <option value=" Price(PKR 43,998) + PKR 6,292  Taxes." >Quadruple Room (PKR 43,998) </option>
        <option value=" Price(PKR 81,996 ) +PKR 11,725  Taxes." > 5 Single bed's (PKR 11,725)</option>
      </select>
        </div>
        <br></br>
        <div >
