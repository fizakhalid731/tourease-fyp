...
import hotelstyle from '@/app/madinastyle/hotel5_regis.module.css';

import Link from 'next/link';

export default function mdhotel5() {
...
const calculation = () => {
  ...
  if (selectedOption === " Price(PKR 81,478) + PKR 11,651 Taxes.") {
    
      const twosingle = jsonData.mdhotel5.two_single_beds + jsonData.mdhotel5.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption ===" Price(PKR 96,292 ) + PKR 13,770 Taxes."){
      const threebed_room = jsonData.mdhotel5.three_single_beds + jsonData.mdhotel5.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Price(PKR 107,403) + PKR 15,359 Taxes."){
      const fourbed_room = jsonData.mdhotel5.four_single_beds + jsonData.mdhotel5.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption ===" Price(PKR 177,770) +PKR 25,421 Taxes."){
      const fivebed_room = jsonData.mdhotel5.five_single_beds + jsonData.mdhotel5.five_single_beds_taxes;
    ...
    <h1> -o( Taiba Front Hotel )o-</h1>
     ...
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Price(PKR 81,478) + PKR 11,651 Taxes." >1 extra large double bed (PKR 81,478) </option>
        <option value=" Price(PKR 96,292 ) + PKR 13,770 Taxes." >Classic Triple Room (PKR 96,292) </option>
        <option value=" Price(PKR 107,403) + PKR 15,359 Taxes." >Quadruple Room (PKR 107,403) </option>
        <option value=" Price(PKR 177,770) +PKR 25,421 Taxes." >extra-large double & 3 Single bed's (PKR 177,770)</option>
      </select>
        </div>
        <br></br>
        <div >
