...
import hotelstyle from '@/app/makkahstyles/regis_hotel4.module.css';

import Link from 'next/link';

export default function Makhotel4_regis() {
...
const calculation = () => {
  ...
  if (selectedOption === " 2 single bed's,  Price(PKR 74,876) + PKR 29,950 Taxes & Fee." ) {
    
      const twosingle = jsonData.hotel4.two_single_beds + jsonData.hotel4.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
      
    }
    
  else if(selectedOption === " 3 single bed's,  Price(PKR 259,792 ) + PKR 12,989 Taxes." ){
      const threebed_room = jsonData.hotel4.three_single_beds + jsonData.hotel4.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Quadruple Room,  Price(PKR 26,021) + PKR 10,410  Taxes." ){
      const fourbed_room = jsonData.hotel4.four_single_beds + jsonData.hotel4.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption === " Family room,  Price(PKR 230,101) +PKR 47,746  Taxes."){
      const fivebed_room = jsonData.hotel4.five_single_beds + jsonData.hotel4.five_single_beds_taxes;
    ...
    <h1> -o( Emaar Grand Hotel )o-</h1>
     ...
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" 2 single bed's,  Price(PKR 74,876) + PKR 29,950 Taxes & Fee." >2 single bed's (PKR 74,876) </option>
        <option value=" 3 single bed's,  Price(PKR 259,792 ) + PKR 12,989 Taxes." >3 single bed's (PKR 259,792) </option>
        <option value=" Quadruple Room,  Price(PKR 26,021) + PKR 10,410  Taxes." >4 single bed's (PKR 26,021) </option>
        <option value=" Family room,  Price(PKR 230,101) +PKR 47,746  Taxes." >Family room /5 single bed's (PKR 230,101)</option>
      </select>
        </div>
        <br></br>
        <div >
