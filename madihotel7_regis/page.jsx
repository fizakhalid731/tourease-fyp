...
import hotelstyle from '@/app/madinastyle/hotel7_regis.module.css';

import Link from 'next/link';

export default function mdhotel7() {
...
const calculation = () => {
  ...
  if (selectedOption === " Price(PKR 81,478) + PKR 13,525 Taxes.") {
    
      const twosingle = jsonData.mdhotel7.two_single_beds + jsonData.mdhotel7.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption ===" Price(PKR 88,885 ) + PKR 14,755 Taxes."){
      const threebed_room = jsonData.mdhotel7.three_single_beds + jsonData.mdhotel7.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption ===" Price(PKR 43,998) + PKR 33,869 Taxes."){
      const fourbed_room = jsonData.mdhotel7.four_single_beds + jsonData.mdhotel7.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption ===" Price(PKR 244,434) +PKR 40,576 Taxes."){
      const fivebed_room = jsonData.mdhotel7.five_single_beds + jsonData.mdhotel7.five_single_beds_taxes;
    ...
    <h1> -o( Al Aqeeq Madinah Hotel )o-</h1>
    </header>
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Price(PKR 81,478) + PKR 13,525 Taxes." >1 large double bed (PKR 81,478) </option>
        <option value=" Price(PKR 88,885 ) + PKR 14,755 Taxes." >Classic Triple Room (PKR 88,885) </option>
        <option value=" Price(PKR 43,998) + PKR 33,869 Taxes." >Quadruple Room (PKR 43,998) </option>
        <option value=" Price(PKR 244,434) +PKR 40,576 Taxes." > 5 Single bed's (PKR 244,434)</option>
      </select>
        </div>
        <br></br>
        <div >
