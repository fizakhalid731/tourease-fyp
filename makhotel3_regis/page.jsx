...
import hotelstyle from '@/app/makkahstyles/regis_hotel3.module.css';

import Link from 'next/link';

export default function Makhotel2_regis() {
...
const calculation = () => {
  ...
  if (selectedOption === " 2 single beds Room,  Price(PKR 28,173) + PKR 1,408 Taxes." ) {
    
      const twosingle = jsonData.hotel3.two_single_beds + jsonData.hotel3.two_single_beds_taxes; 
    ...
    else if(selectedOption === " 3 single bed's,  Price(PKR 97,037 ) + PKR 4,851 Taxes." ){
      const threebed_room = jsonData.hotel3.three_single_beds + jsonData.hotel3.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Quadruple Room,  Price(PKR 35,626) + PKR 1,781 Taxes." ){
      const fourbed_room = jsonData.hotel3.four_single_beds + jsonData.hotel3.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption === " Family room,  Price(PKR 121,583) +PKR 6,079  Taxes."){
      const fivebed_room = jsonData.hotel3.five_single_beds + jsonData.hotel3.five_single_beds_taxes;
    ...
    <h1> -o( Snood Al-Rayyan Hotel )o-</h1>
    </header>
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" 2 single beds Room,  Price(PKR 28,173) + PKR 1,408 Taxes." >2 single beds (PKR 28,173) </option>
        <option value=" 3 single bed's,  Price(PKR 97,037 ) + PKR 4,851 Taxes." >3 single bed's (PKR 97,037) </option>
        <option value=" Quadruple Room,  Price(PKR 35,626) + PKR 1,781 Taxes." >Quadruple Room /4 single bed's (PKR 35,626) </option>
        <option value=" Family room,  Price(PKR 121,583) +PKR 6,079  Taxes." >Family room /5 single bed's (PKR 121,583)</option>
      </select>
        </div>
        <br></br>
        <div >
