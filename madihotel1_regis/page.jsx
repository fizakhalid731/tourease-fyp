...
import hotelstyle from '@/app/madinastyle/hotel1_regis.module.css';

import Link from 'next/link';

export default function mdhotel1() {
...
const calculation = () => {
  ...
  if (selectedOption === " Price(PKR 155,549) + PKR 32,276 Taxes.") {
    
      const twosingle = jsonData.mdhotel1.two_single_beds + jsonData.mdhotel1.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);
    }
    
  else if(selectedOption === " Price(PKR 162,956 ) + PKR 33,813 Taxes."){
      const threebed_room = jsonData.mdhotel1.three_single_beds + jsonData.mdhotel1.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room );
    
    }
    else if(selectedOption === " Price(PKR 177,770) + PKR 36,887 Taxes."){
      const fourbed_room = jsonData.mdhotel1.four_single_beds + jsonData.mdhotel1.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption === " Price(PKR 318,505) +PKR 66,090 Taxes."){
      const fivebed_room = jsonData.mdhotel1.five_single_beds + jsonData.mdhotel1.five_single_beds_taxes;
    ...
    <h1> -o(Anwar Al Madinah Mövenpick)o-</h1>
    </header>
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Price(PKR 155,549) + PKR 32,276 Taxes." >2 Single bed's (PKR 155,549) </option>
        <option value=" Price(PKR 162,956 ) + PKR 33,813 Taxes." >Classic Triple Room (PKR 162,956) </option>
        <option value=" Price(PKR 177,770) + PKR 36,887 Taxes." >Quadruple Room (PKR 177,770) </option>
        <option value=" Price(PKR 318,505) +PKR 66,090 Taxes." >extra-large double & 3 Single bed's (PKR 318,505)</option>
      </select>
        </div>
        <br></br>
        <div >
