...
import hotelstyle from '@/app/makkahstyles/regis_hotel1.module.css';
...
const calculation = () => {
  ...
  if (selectedOption === ' Price(PKR 51,054) + PKR 10,594 Taxes & Breakfast included.') {
    
      const twosingle = jsonData.hotelone.two_single_beds + jsonData.hotelone.two_single_beds_taxes; 
       const totaltwpsingle = twosingle * numberOfNights;
        setTotalCost(totaltwpsingle);

    }
    
  else if(selectedOption === " Price(PKR 123,137 ) + PKR 6,156 Taxes."){
      const threebed_room = jsonData.hotelone.three_single_beds + jsonData.hotelone.three_single_beds_taxes;
      const totalthreebed_room = threebed_room * numberOfNights;
      setTotalCost(totalthreebed_room);
    
    }
    else if(selectedOption === " Price(PKR 131,000) + PKR 6550 Taxes."){
      const fourbed_room = jsonData.hotelone.four_single_beds + jsonData.hotelone.four_single_beds_taxes;
      const totalfourbed_room = fourbed_room * numberOfNights;
      setTotalCost(totalfourbed_room );
    
    }
    else if(selectedOption === " Price(PKR 139,442) +PKR 28,934  Taxes."){
      const fivebed_room = jsonData.hotelone.five_single_beds + jsonData.hotelone.five_single_beds_taxes;
    ...
    <h1> -o( Pullman ZamZam Makkah )o-</h1>
    </header>
     <form onSubmit={handleSubmit} className={hotelstyle.hotelform}>
       ...
       <select  className={hotelstyle.formGroupselect} value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        ...
        <option value=" Price(PKR 51,054) + PKR 10,594 Taxes & Breakfast included." >2 Single bed (PKR 51,054) & Breakfast included </option>
        <option value=" Price(PKR 123,137 ) + PKR 6,156 Taxes." >1 Single bed and 1 Twin bed's (PKR123,137) </option>
        <option value=" Price(PKR 131,000) + PKR 6550 Taxes." >2 Twin bed (PKR 131,000) </option>
        <option value=" Price(PKR 139,442) +PKR 28,934  Taxes." >Classic Presidential Suite (PKR 139,442)</option>
      </select>
        </div>
        <br></br>
        <div >
